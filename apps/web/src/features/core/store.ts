...
const POLYTOPE_DIRTY: ViewportDirtyFlags = {
  ...
  constraints: true,
  objective: true,
  // the in-polytope heatmap masks against the polytope, so it rebuilds with it
  objectiveHeatmap: true,
};
...
const objectiveDirty = (s: State): ViewportDirtyFlags => (s.is3DMode || s.isTransitioning3D ? { polytope: true, objective: true, objectiveHeatmap: true } : { objective: true, objectiveHeatmap: true });
...
const FIELD_DIRTY: Partial<Record<keyof State, (s: State) => ViewportDirtyFlags>> = {
  ...
  objectiveHidden: () => ({ objective: true, objectiveHeatmap: true }),
  objectiveHeatmapEnabled: () => ({ objectiveHeatmap: true }),
  objectiveHeatmapCell: () => ({ objectiveHeatmap: true }),
...
export type State = {
  ...
  objectiveHeatmapEnabled: boolean;
  objectiveHeatmapCell: number;
...
const initialState: State = {
  ...
  objectiveHeatmapEnabled: false,
  objectiveHeatmapCell: 1,
...
export function updateIteratePathsWithTrace(path: IteratePath, phasesArray?: number[], restartIndicesArray?: number[], ellipsoids?: EllipsoidPath | null, localizingSets?: LocalizingSetPath | null): void {
  const state = getState();
...
function buildIterateStatePatch(path: IteratePath, phasesArray: number[] | undefined, restartIndicesArray: number[] | undefined, objectiveSnapshot: PointXY | null, ellipsoids: EllipsoidPath | null = null, localizingSets: LocalizingSetPath | null = null): Partial<State> {
